...
export async function guardarPedidoOrden(_prev, fd) {
  ...
  try {
    ...
    if (id) {
      await q(
        `UPDATE pedido_orden SET folio=?, fecha_pedido=?, proveedor=?, monto_total_usd=?, notas=? WHERE id=?`,
        [folio, fecha_pedido, proveedor, monto_total_usd, notas, id])
      pedidoId = id
    } else {
      const r = await q(
        `INSERT INTO pedido_orden (folio, fecha_pedido, proveedor, monto_total_usd, notas)
         VALUES (?,?,?,?,?)`,
        [folio, fecha_pedido, proveedor, monto_total_usd, notas])
      pedidoId = r.insertId
    }
...
export async function marcarPedidoEnviado(fd) {
  ...
  const fecha_envio = fd.get('fecha_envio') || new Date().toISOString().split('T')[0]
  
  try {
    await q(
      `UPDATE pedido_orden SET estado='EN_TRANSITO', guia_envio=?, fecha_envio=? WHERE id=?`,
      [guia_envio, fecha_envio, id])
  } catch (e) {
    return { error: e.message }
  }
  revalidatePath('/pedidos')
...
export async function recibirPedido(fd) {
  ...
  try {
    await tx(async (run) => {
      ...
      const lineas = await run('SELECT * FROM pedido_orden_linea WHERE pedido_orden_id = ?', [id])
        ...
        if (linea.sku_id) {
          await run(
            `INSERT INTO movimiento (fecha, sku_id, tipo, cantidad, notas)
             VALUES (?, ?, 'COMPRA', ?, ?)`,
            [fecha_recepcion, linea.sku_id, linea.cantidad, `Pedido recibido - ${linea.descripcion}`])
        }
      }
      
      // Actualizar estado del pedido
      await run(
        `UPDATE pedido_orden SET estado='RECIBIDO', fecha_recepcion=? WHERE id=?`,
        [fecha_recepcion, id])
    })
  } catch (e) {
    return { error: e.message }
