...
export async function eliminarCruce(_prev, fd) {
  const id = num(fd.get('id'))
  if (!id) return { error: 'ID de cruce inválido' }

  try {
    // Verificar si tiene paquetes asociados
    const paquetes = await q(`SELECT COUNT(*) as count FROM paquete WHERE cruce_id = ?`, [id])

    if (paquetes[0].count > 0) {
      return {
        error: `No se puede eliminar: el cruce tiene ${paquetes[0].count} paquete(s) asociado(s). Primero elimina o reasigna los paquetes.`
      }
    }

    await q(`DELETE FROM cruce WHERE id = ?`, [id])
  } catch (e) {
    return { error: e.message }
  }
